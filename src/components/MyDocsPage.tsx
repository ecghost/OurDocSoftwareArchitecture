...
const MyDocsPage: React.FC = () => {
  ...
  const handleRemovePermission = async (userId: string) => {
    if (!selectedDoc) return;
    try {
      const res = await removeUserPermission({
        room_id: selectedDoc.room_id,
        user_id: userId,
      });
      console.log(res);
  ...
  const handleChangePermission = async (userId: string, permission: 3 | 2) => {
    ...
    try {
      ...
      const newPermissions = {
        ...
        [userId]: {
          ...(selectedDoc.permissions[userId] || {
            userId,
            username: "",
            email: "",
          }),
  ...
  const handleRenameSubmit = async () => {
    ...
    try {
      ...
      const res = await renameRoom({
        room_id: selectedDocId,
        room_name: newName,
      });
      console.log(res);
      const newDocs = docs.map((d) =>
        d.room_id === selectedDocId ? { ...d, room_name: newName } : d
      );
      console.log(newDocs);
      setDocs(newDocs);
      setRenameModalVisible(false);
                ...
                <Card title="危险操作">
                  <Space direction="vertical">
                    <Popconfirm
                      ...
                      onConfirm={() =>
                        handleDeleteDocument(selectedDoc?.room_id)
                      }
                    >
                ...
                <Card title="文档信息">
                  <div>
                    ...
                    <div style={{ marginBottom: 8 }}>
                      {selectedDoc.create_time}
                    </div>

                    <div>
                      <Text type="secondary">当前公开性</Text>
                    </div>
                    <div style={{ marginBottom: 8 }}>
                      <Tag>
                        {selectedDoc.overall_permission === PRIVATE_VIEW &&
                          "私密"}
                        {selectedDoc.overall_permission === EVERYONE_READ &&
                          "所有人可编辑"}
                        {selectedDoc.overall_permission === EVERYONE_EDIT &&
                          "所有人可阅读"}
                        {selectedDoc.overall_permission === PARTIAL &&
                          "部分人可见"}
                      </Tag>
                    </div>
                  </div>
        ...
        <div style={{ display: "flex", gap: 16 }}>
          ...
          <div style={{ width: 320 }}>
            ...
            <List
              ...
              renderItem={(user) => {
                const selected = selectedUsersInModal.some(
                  (s) => s.user.id === user.id
                );
                // disable if user already in doc permissions
                const alreadyHas = selectedDoc
                  ? !!selectedDoc.permissions[user.id]
                  : false;
                return (
                  <List.Item
                    style={{
                  ...
                  >
                    ...
                    />
                    <div>{selected ? <Tag>已选择</Tag> : null}</div>
                  </List.Item>
                );
              }}
            />
              ...
              renderItem={(item) => (
                ...
                >
                  <List.Item.Meta
                    ...
                    description={
                      <div style={{ fontSize: 12 }}>{item.user.email}</div>
                    }
                  />
                  <div>
                    <Space>
                      <Button
                        ...
                        onClick={() =>
                          setPermissionForSelectedUser(item.user.id, 3)
                        }
                      >
                        可编辑
                      </Button>
                      <Button
                        ...
                        onClick={() =>
                          setPermissionForSelectedUser(item.user.id, 2)
                        }
                      >
                        只读
                      </Button>
